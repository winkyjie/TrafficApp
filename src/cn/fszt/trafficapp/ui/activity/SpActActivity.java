...
import cn.fszt.trafficapp.adapter.SpActAdapter;
...
public class SpActActivity extends Activity{
	
	private LayoutInflater inflater;
	private List<CarlifeAndHuodongData> arrays;
	private PullDownView pullDownView;
	private ScrollOverListView listView;
	private SpActAdapter adapter;
	private String delmpl_url,space_huodong_url,uid;
	...
	private List<CarlifeAndHuodongData> sql_arrays;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		sql_arrays = mgr.queryS_huodongByConnected_uid(uid+"");
		
		if(sql_arrays.size()==0){  //当数据库没有数据时，从网络更新并插入数据库
	       	initArrays(new Handler(){
	           	@Override
	           	public void handleMessage(Message msg) {
	           		arrays = (List<CarlifeAndHuodongData>) msg.obj;
	           		adapter = new SpActAdapter(SpActActivity.this,arrays,inflater,delmpl_url,uid,densityDpi);
	                listView.setAdapter(adapter);
	           		adapter.notifyDataSetChanged();
	           		pullDownView.notifyDidDataLoad(false);
	           		
	           		if(arrays!=null){
	           			mgr.addS_huodong(arrays);
	   		...
	   		initArrays(new Handler(){
	           	...
	           	public void handleMessage(Message msg) {
	           		...
	           		if(arrays==null){  //网络更新失败时，还是显示数据库的历史数据
	           			...
	           			mgr.deleteS_huodongByconnected_uid(uid+"");
						mgr.addS_huodong(arrays);
						pb_carlife.setVisibility(View.GONE);
	           		}
	           		adapter = new SpActAdapter(SpActActivity.this,arrays,inflater,delmpl_url,uid,densityDpi);
        ...
        pullDownView.setOnPullDownListener(new OnPullDownListener() {
			...
			public void onLoadMore() {
				getLoadMore(new Handler(){
					@Override
					public void handleMessage(Message msg) {
						if((List<CarlifeAndHuodongData>)msg.obj!=null){
							for(CarlifeAndHuodongData s :(List<CarlifeAndHuodongData>)msg.obj){
								arrays.add(s);
							}
						}
						adapter.notifyDataSetChanged();
						pullDownView.notifyDidLoadMore((List<CarlifeAndHuodongData>)msg.obj==null);
					}
				});
			}
		});
	...
	private void initView() {
    	...
    	space_huodong_url = getResources().getString(R.string.server_url)+getResources().getString(R.string.space_huodong_url);
	...
	private void initArrays(final Handler handler) {
		new Thread(new Runnable() {
			List<CarlifeAndHuodongData> lst = new ArrayList<CarlifeAndHuodongData>();
			@Override
			public void run() {
				try{  
	 ...
	 * @throws Exception
	 */
	private List<CarlifeAndHuodongData> getDataFromNetwork(int start,int end) throws Exception{
		List<CarlifeAndHuodongData> lst = new ArrayList<CarlifeAndHuodongData>();
		String resultString = HttpUtil.GetStringFromUrl(space_huodong_url+"&connected_uid="+uid+"&startindex="+start+"&endindex="+end);
		if(resultString != null&&!"".equals(resultString)){
			Type listType = new TypeToken<List<CarlifeAndHuodongData>>(){}.getType();
	        Gson gson = new Gson();
