...
import cn.fszt.trafficapp.widget.ScrollOverListView.OnScrollOverListener;
...
public class PullDownView extends LinearLayout implements OnScrollOverListener,
	...
	/**
	 * 是否开启自动获取更多</br> 自动获取更多，并在触摸到达底部的时候自动刷新
	 * 
	 * @param index
	 *            倒数第几个触发
	 */
	...
	private void initHeaderViewAndFooterViewAndListView(Context context) {
		...
		mDefaultHeaderViewHeight = getResources().getDimensionPixelSize(
				R.dimen.pulldown_headerview_height);
		mMoveDeviation = getResources().getDimensionPixelSize(
				R.dimen.pulldown_move_deviation);
		...
		/*
		 * ScrollOverListView 同样是考虑到基本都要使用，所以放在这里 同时因为，需要它的监听事件
		 */
		...
		mListView.setFastScrollEnabled(false); // 是否显示快速滑动块
		mListView.setVerticalScrollBarEnabled(true); // 是否隐藏默认的滚动条
		 ...
		 * animation.setDuration(50); set.addAnimation(animation);
		 * 
		 * animation = new TranslateAnimation( Animation.RELATIVE_TO_SELF,
		 * 0.0f,Animation.RELATIVE_TO_SELF, 0.0f, Animation.RELATIVE_TO_SELF,
		 * -1.0f,Animation.RELATIVE_TO_SELF, 0.0f ); animation.setDuration(100);
	...
	/**
	 * 更新头部文件</br> 如果刷新中显示Loading， 如果拖动中，显示拖动效果， 否则显示更新时间。
	 */
	...
	/**
	 * 更新脚部文件</br> 正常显示"更多"， 如果没有更多的数据就显示"已加载完全部"， 如果加载中就显示"加载中..."。
	 */
	...
	class ShowHeaderViewTask extends TimerTask {
		...
		public void run() {
			...
			if (mHeaderIncremental > mDefaultHeaderViewHeight) {
				...
				mUIHandler.sendEmptyMessage(WHAT_SET_HEADER_HEIGHT);
				if (mIsDidLoad
						&& (state & STATE_REFRESHING) != STATE_REFRESHING) {
					state |= STATE_REFRESHING;
					mUIHandler.post(new Runnable() {
