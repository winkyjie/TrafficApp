...
//                int x = (int)(((r.left+r.right)/2)-(this.p.measureText(p.getName())/2));
                int x = left;
                int y = getHeight()-25;
                StaticLayout layout = new StaticLayout(p.getName(),textPaint,right-left,Alignment.ALIGN_NORMAL,1.1F,0.0F,true);
                canvas.save();
                canvas.translate(x, getHeight()-bottomPadding+15);
                layout.draw(canvas);  
                canvas.restore();
