...
public class ViewPagerByList extends Activity {
	...
	class SamplePagerAdapter extends PagerAdapter {
		...
		private ArrayList<String> sDrawables = imgurl; 
		...
		public View instantiateItem(ViewGroup container, final int position) {
			...
			pv_viewpager.setOnLongClickListener(new OnLongClickListener() {
				...
				public boolean onLongClick(View v) {
					PopupMenu menu = new PopupMenu(ViewPagerByList.this);
						...
						public void onItemSelected(MenuItem item) {
							switch(item.getItemId()){
								case SAVE2PHONE:
									new Thread(){
										public void run() {
											Bitmap bm = PhotoUtil.getBitmapFromUrl(sDrawables.get(position));
				...
				imageLoader.displayImage(sDrawables.get(position), pv_viewpager, options,new ImageLoadingListener() {
	...
	Handler handler = new Handler(){
		public void handleMessage(android.os.Message msg) {
			if(msg.what==SAVEPHOTO){
				if(save){
					Toast.makeText(ViewPagerByList.this, "图片已保存至"+_imageDir+" 文件夹", Toast.LENGTH_SHORT).show();
				}
			}
		}
	};
